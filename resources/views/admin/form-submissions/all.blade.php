<x-admin-layout>
    ...
    <form method="GET" action="{{ route('admin.form-submissions.all') }}" class="flex flex-wrap gap-4 mb-4">
        <div>
            <label class="block text-xs font-medium text-gray-500 mb-1">{{ __('Form') }}</label>
            <select name="form_id" onchange="this.form.submit()" class="rounded-md border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                <option value="">{{ __('All Forms') }}</option>
                @foreach ($forms as $form)
                    <option value="{{ $form->id }}" @selected($selectedFormId === $form->id)>{{ $form->name }}</option>
                @endforeach
            </select>
        </div>

        <div>
            <label class="block text-xs font-medium text-gray-500 mb-1">{{ __('Service') }}</label>
            <select name="service" onchange="this.form.submit()" class="rounded-md border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                <option value="">{{ __('All Services') }}</option>
                @foreach ($services as $service)
                    <option value="{{ $service }}" @selected($selectedService === $service)>{{ ucwords(str_replace('_', ' ', $service)) }}</option>
                @endforeach
            </select>
        </div>

        <div>
            <label class="block text-xs font-medium text-gray-500 mb-1">{{ __('Capture Status') }}</label>
            <select name="capture_status" onchange="this.form.submit()" class="rounded-md border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                <option value="">{{ __('All') }}</option>
                @foreach (\App\Models\FormSubmission::CAPTURE_STATUSES as $captureStatus)
                    <option value="{{ $captureStatus }}" @selected($selectedCaptureStatus === $captureStatus)>{{ ucfirst($captureStatus) }}</option>
                @endforeach
            </select>
        </div>
    </form>
